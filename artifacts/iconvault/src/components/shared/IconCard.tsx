    ...
    <>
      <Link href={`/icons/${icon.slug}`} className="group block aspect-square">
        <div className="nb-card h-full flex flex-col relative overflow-hidden transition-transform duration-200 group-hover:-translate-y-1 group-hover:shadow-[6px_6px_0_#0A0A0A]">
          ...
          <div className="flex-1 flex items-center justify-center p-[18%] bg-card border-b-[3px] border-foreground relative min-h-0">
          ...
          <div className="px-1.5 sm:px-2.5 py-1.5 bg-secondary flex items-center justify-between gap-1">
            <button
              onClick={handleLike}
              className="flex items-center gap-0.5 hover:text-accent transition-colors min-w-0"
              data-testid={`btn-like-${icon.id}`}
            >
              <Heart className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
              <span className="font-mono text-[10px] sm:text-xs font-bold">{likes}</span>
            </button>
            <span className="font-mono text-[9px] sm:text-[10px] font-bold opacity-60 flex items-center gap-0.5 shrink-0">
              <Download className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> {icon.downloads}
            </span>
          </div>
        </div>
      </Link>
