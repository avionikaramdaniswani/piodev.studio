...
import { Shield, Users, Upload, BarChart2, Download, Heart, Grid, LogOut, ChevronRight } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto py-6 flex flex-col gap-6">
      ...
      <div className="nb-card overflow-hidden">
        ...
        <div className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div
              className="w-12 h-12 border-[3px] border-foreground shadow-[3px_3px_0_#0A0A0A] flex items-center justify-center font-black text-sm shrink-0"
          ...
          <button
            onClick={signOut}
            className="nb-btn px-4 py-2 text-sm font-black flex items-center gap-2"
            style={{ background: "#FF6B35", color: "white" }}
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "PENGGUNA", value: users.length > 0 ? users.length + "+" : "–", icon: <Users className="w-6 h-6" />, bg: "#00E676" },
        ].map(s => (
      ...
      {role === "admin" && (
        <div className="nb-card p-6">
          ...
          {loadingUsers ? (
            ...
            <p className="font-mono text-sm opacity-50">Belum ada data pengguna. Pastikan SQL RBAC sudah dijalankan di Supabase.</p>
          ) : (
              ...
              <table className="w-full text-sm font-mono">
                <thead>
                  <tr className="border-b-[3px] border-foreground">
                    ...
                    <th className="text-left py-2 pr-4 font-black">BERGABUNG</th>
                    <th className="text-left py-2 font-black">PERAN</th>
                  </tr>
                </thead>
                <tbody>
                  {users.map(u => (
                    <tr key={u.id} className="border-b-[2px] border-foreground/10 hover:bg-secondary/50">
                      ...
                      <td className="py-3 pr-4 opacity-50 whitespace-nowrap">
